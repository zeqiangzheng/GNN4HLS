...
class Element:
    ...
    def vector(self):
        return [self.id, self.ty, self.name, self.bitwidth, self.opcode, \
                self.delay, self.line_no, self.pragma]

class Port(Element):
    def __init__(self, port=None, id_space=None, pragma_lines=None) -> None:
        super().__init__()
        if port and id_space:
            ...
            if pragma_lines and (self.line_no in pragma_lines):
                self.pragma = pragma_lines[self.line_no]

class Node(Element):
    def __init__(self, node=None, id_space=None, pragma_lines=None) -> None:
        super().__init__()
...
class Const(Element):
    def __init__(self, const=None, id_space=None, pragma_lines=None) -> None:
        super().__init__()
...
class Block(Element):
    def __init__(self, block=None, id_space=None, pragma_lines=None) -> None:
        super().__init__()
...
def parse(adb_file, args, rootpath=""):
    
    if hasattr(args, 'pragma_lines'):
        pragma_lines = args.pragma_lines
    else:
        pragma_lines = None
    ...
    out_csv = [['id', 'type', 'name', 'bitwidth', 'opcode', 'delay', \
                'line_no', 'pragma']]
    ...
    if args.json:
        import json
        with open(rootpath + 'graph_dic.json', 'w') as f:
            json.dump(out_json, f, indent=4)

        if args.pickle:
            with open(rootpath + 'data.pkl', 'wb') as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--out_npy', help="output *.npy file",
                        default="out.npy")
