...
from adb_parser_modified_directives import parse
from  collect_loop_info import collect_loop_info
import time
import re
...
for i in range(from_index, to_index+1): 
  project_0 = str(i).zfill( c_namelen )
    
  # Checking C file
  c_file = c_folder + "/" + project_0 + ".c"
  if os.path.exists(c_file):
    ...
    if num_lines <= min_c_lines:
      print("Skipping " + project_0 + ".c because <= 11 lines")      
      continue
    for i in range(num_lines):
      line = lines[i]
      if line[:7] == "extern ":  # for 'extern' synthesis error
        lines[i] = "       " + line[7:]  # to avoid altering filesize
        print("Removing 'extern' prefix from line " + str(i) + " in " + project_0 + ".c" )
        rewrite_file = True          
      if line[-11:] == " fn1(void)":
        stop_outer = True
        break
    ...
    f.close()
    if stop_outer:
      print("Skipping " + project_0 + ".c because fn1(void) (no argument)")
      continue
  ...
  pragma_ranges = collect_loop_info(c_file)
  ...
  for loop in res:
    ...
    pragma_range = pragma_ranges['loop' + str(counter)]
    pragma_lines = {}
    for i in range(pragma_range[0], pragma_range[1]+1):
      pragma_lines[i] = 1

    project = project_0 + f'_{counter}'
    path = folder + "/" + project
    tmp_prj = tmp + "/" + project

    if os.path.isdir(path) or os.path.isdir(tmp_prj):
      print("Skipping " + project + " (folder exists already)")
      break #continue  # try next index
    ...
    tcl.write(f'set_directive_pipeline {res[counter-1]}\n')
    ...
    # New: moved all this code block here to avoid multiprocess conflicts in multigen.py
    ...
    # NOT necessary anymore, we write and close inside each loop now
    #hls_file.flush()
